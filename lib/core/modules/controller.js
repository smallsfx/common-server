  ...
  Object.keys(mapping).map(function(action) {
    ...
    if (api_method = mapping[action]) {
      ...
      if (api_method.method == undefined) {
        if (action == "delete") {
          api_method.method = "post"; //"delete";
          // if (api_method.url == undefined) api_method.url = "";
        } else if (action == "update") {
          api_method.method = "post"; //"put";
          // if (api_method.url == undefined) api_method.url = "";
        } else if (action == "insert") {
          api_method.method = "post"; //"post";
          // if (api_method.url == undefined) api_method.url = "";
        } else {
      ...
      try {
        ...
        console.log(`${ml},${ul},${api_method.url}`);
...
module.exports = {
  ...
  "run": (config, app) => {
    ...
    if (config && config.catalog && config.catalog.controller) {
      const controller_path = config.catalog.controller;
      if (Array.isArray(controller_path)) {
        controller_path.forEach((_path) => {
          paths = paths.concat(fsh.scan(fsh.join(config.rootPath, _path)));
        });
      } else {
        var rootPath = fsh.join(config.rootPath, controller_path);
