...
module.exports = {
  ...
  "mapping": {
    ...
    "login": { description: '', auth: false, method: 'post' },
  ...
  "getCaptcha": (req, res) => {
    res.send('{"c":0,"data":{"captch_url":"http://192.168.8.103:3000/logo.png","sessionId":"test-nodejs-wangxin"}}');
  },
...
/**
离线统计查询轮询接口：offLineStatLunXun
传：sessionId--str--
返回：和离线统计一样

离线告警取消接口：offLineIgnore
传：sessionId--str--,id--str--目标的短ID（sid）,单个目标，不支持多个目标
返回：c:0,data:{}
[
  {"sid":"1","sim":"13700043840","tName":"c2pm5rWL6K+VMw==","gName":"c2pm5rWL6K+VMw==","gTime":"1","day":"3"},
  {"sid":"2","sim":"13700043840","tName":"c2pm5rWL6K+VMw==","gName":"c2pm5rWL6K+VMw==","gTime":"1","day":"3"},
 ...
 String sid = _help.getAsString("short_id");
                    String sim = ua.getTargetList().get(sid).getSu_mdn();
                    String tName = ua.getTargetList().get(sid).getName();
                    String gName = ua.getTargetList().get(sid).getGroupName();
                    JSONObject tempJSON = new JSONObject();
                    tempJSON.put("sim", sim);
                    tempJSON.put("tName", Base64Codec.encode(tName));
                    tempJSON.put("gTime", _help.getAsInteger("time"));
                    tempJSON.put("gName", StringUtils.isNullOrBlank(gName) ? "" : Base64Codec.encode(gName));
                    tempJSON.put("day", timeConvert(_help.getAsInteger("time"), day));
 */